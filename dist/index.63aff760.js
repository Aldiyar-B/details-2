...
let citiesList = (0, _storageJs.storage).getFavoriteCities();
render();
...
function addCity(e) {
    ...
    if (citiesList.length > 0) {
        if (citiesList.includes(currenCity)) deleteFavoriteCity(currenCity);
        else if (currenCity !== "") {
            citiesList.push(currenCity);
            (0, _storageJs.storage).saveFavoriteCities(citiesList);
        }
...
function deleteCity() {
    ...
    /*
	function iterationList(list, i = 0) {

		let newCitiesList = [];
		if (list.length === i) {
			return list;
		} else {
			let currenCity = .parentNode.textContent;
			if (currenCity !== list[i]) {
				newCitiesList.push(list[i]);
				iterationList(list, i + 1);
		...
		return newCitiesList = citiesList;
	}
	iterationList(citiesList);*/ (0, _storageJs.storage).saveFavoriteCities(citiesList);
    render();
}
...
function render() {
    ...
    citiesList = (0, _storageJs.storage).getFavoriteCities();
    if (citiesList.length > 0) citiesList.forEach((city)=>{
...
function replace(currenCity) {
    ...
    for (let city of cities)city.textContent = currenCity;
    (0, _storageJs.storage).addLastCityName(currenCity);
}
function openFavoritCity(e) {
    let currenCity = e.target;
    console.log(currenCity.textContent);
    if (currenCity) sendRequest(currenCity.textContent);
...
async function sendRequest(text) {
    ...
    try {
        ...
        //console.log(data);
        UI_ELEMENTS.NOE_TEMPERATURE.textContent = jsonData.main.temp.toFixed(0) + "\xb0";
        (0, _storageJs.storage).getLastCityName();
        (0, _storageJs.storage).addLastCityName(jsonData.name);
